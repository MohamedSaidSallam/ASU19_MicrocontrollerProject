...
#define SSID_NAME "MohamedXyz"
#define PASSKEY "FreeeInternet"
...
void SearchCheck(char letter)
{
  if (SearchLooking)
  {
    if (SearchString[SearchIndex] == lc(letter))
    { // match letter?
      SearchIndex++;
      if (SearchString[SearchIndex] == 0)
      { // match string?
        SearchFound = true;
        SearchLooking = false;
      }
...
void ServerResponseSearchCheck(char letter)
{
  ...
  {
    if (ServerResponseSearchString[ServerResponseSearchIndex] == lc(letter))
    { // match letter?
      ServerResponseSearchIndex++;
      if (ServerResponseSearchString[ServerResponseSearchIndex] == 0)
      { // match string?
...
void ESP8266_InitUART(uint32_t baud, int echo)
{
  ...
  SYSCTL_RCGC1_R |= SYSCTL_RCGC1_UART1; // Enable UART1
  while ((SYSCTL_RCGC1_R & SYSCTL_RCGC1_UART1) == 0)
  {
  };
  SYSCTL_RCGC2_R |= SYSCTL_RCGC2_GPIOB; // Enable PORT B clock gating
  ...
  GPIO_PORTB_AMSEL_R &= ~0x03; //! i added this
  ...
  UART1_CTL_R |= UART_CTL_UARTEN | UART_CTL_RXE | UART_CTL_TXE;                    // Set UART1 enable bit
...
void DelayMs(uint32_t n)
{
  ...
  while (n)
  {
    ...
    // time = 1333; // 1msec, tuned at 80 MHz
    while (time)
    {
      time--;
    }
...
void DelayMsSearching(uint32_t n)
{
  ...
  while (n)
  {
    ...
    // time = 813; // 1msec, tuned at 80 MHz
    while (time)
    {
      time--;
      if (SearchFound)
...
void registerOnCloud(int id){
  char fetch[] = "GET /registerAttendance?id=13 HTTP/1.1\r\nHost: us-central1-microcontrollerproject-69ca4.cloudfunctions.net\r\n\r\n";  
  sprintf((char *)TXBuffer, "AT+CIPSTART=\"TCP\",\"%s\",80\r\n", "us-central1-microcontrollerproject-69ca4.cloudfunctions.net");
  ESP8266SendCommand(TXBuffer); // open and connect to a socket
  DelayMs(60); // ! to all of them ?
  sprintf((char *)TXBuffer, "AT+CIPSEND=%d\r\n", strlen(fetch));
  ESP8266SendCommand(TXBuffer);
  DelayMs(60); // ! to all of them ?
  ESP8266SendCommand(fetch);
...
void initModule(void){
  ESP8266SendCommand("AT+RST\r\n");
  DelayMs(200); // ! to all of them ?
  sprintf((char *)TXBuffer, "AT+CWMODE=%d\r\n", ESP8266_WIFI_MODE_CLIENT);
  ESP8266SendCommand((const char *)TXBuffer);
  DelayMs(60); // ! to all of them ?
  sprintf((char *)TXBuffer, "AT+CWJAP=\"%s\",\"%s\"\r\n", SSID_NAME, PASSKEY);
  ESP8266SendCommand((const char *)TXBuffer);
...
void initPorts(void){
  ESP8266_InitUART(115200, true); // baud rate, no echo to UART0
  ESP8266_EnableRXInterrupt();
  SearchLooking = false;
  SearchFound = false;
  ServerResponseSearchLooking = 0; // not looking for "+IPD"
